...
export async function renderColecta() {
    ...
    <div class="section-header">
      <h1 class="section-title">📦 Colecta</h1>
      <p class="section-subtitle">Envíos por Mercado Envíos (Colecta)</p>
    </div>
    ...
    try {
      ...
      <div class="section-header flex-between">
        ...
        <div style="display:flex;gap:8px;">
          <button class="btn btn-success btn-sm" id="btn-mark-all-colecta">✓ Marcar todo encontrado</button>
        </div>
      </div>
        ...
        <div class="card mb-md">
          ...
          <div style="display:flex;gap:8px;flex-wrap:wrap;">
            ${Object.entries(carriers).sort((a, b) => b[1] - a[1]).map(([name, count]) => `
              <span class="badge badge-colecta" style="font-size:13px;padding:6px 14px;">🚚 ${name}: ${count}</span>
            `).join('')}
          </div>
        </div>
      ` : ''}
      ...
      <div id="colecta-picking-items">
        ${colectaPicking.length === 0 ? `
          <div class="empty-state">
            ...
            <p class="empty-state-text">No hay envíos Colecta.<br>Subí etiquetas primero.</p>
          </div>
        ` : ''}
      </div>
    `;
