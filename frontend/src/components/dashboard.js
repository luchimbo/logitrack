...
export async function renderDashboard() {
  ...
  section.innerHTML = `
    <div class="section-header">
      ...
      <p class="section-subtitle">Resumen del día</p>
    </div>
  ...
  try {
    ...
    if (data.total_packages === 0) {
      section.innerHTML = `
        ...
        <div class="empty-state">
          ...
          <p class="empty-state-text">No hay datos para mostrar.<br>Subí etiquetas para ver el dashboard.</p>
        </div>
      `;
      return;
    }
    ...
    section.innerHTML = `
      <div class="section-header">
        <h1 class="section-title">📊 Dashboard</h1>
        <p class="section-subtitle">Resumen del día — ${new Date().toLocaleDateString('es-AR')}</p>
      </div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;">
        ...
        <div class="card" style="border-top: 3px solid #f59e0b;">
          <h3 style="margin-bottom:16px;font-size:16px;font-weight:700;">📦 Colecta — ${colectaCount} paquetes</h3>
          ${colectaCount > 0 ? `
      ...
      return `
                  <div class="chart-bar-row">
                    <div class="chart-bar-label">🚚 ${carrier}</div>
          ...
          ` : '<p style="color:var(--text-muted);font-size:14px;">No hay envíos colecta</p>'}
        </div>

        <!-- FLEX Section -->
        <div class="card" style="border-top: 3px solid var(--accent);">
          <h3 style="margin-bottom:16px;font-size:16px;font-weight:700;">🚀 Flex — ${flexCount} paquetes</h3>
          ${flexCount > 0 ? `
            <p style="color:var(--text-secondary);font-size:14px;margin-bottom:8px;">
              Envíos Flex pendientes de despacho por transportista propio
            </p>
          ` : '<p style="color:var(--text-muted);font-size:14px;">No hay envíos flex</p>'}
        </div>
      </div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-top:20px;">
        <!-- Status Chart -->
        <div class="card">
          <h3 style="margin-bottom:20px;font-size:16px;font-weight:700;">Estado de Envíos</h3>
          <div class="chart-bar-container">
