...
const PARTIDOS = [
  // ÔöÇÔöÇ CAPITAL ÔöÇÔöÇ
  { id: 'capital_federal', name: 'CABA', zone: 'capital' },

  // ÔöÇÔöÇ GBA 1 (inner ring) ÔöÇÔöÇ
  { id: 'san_isidro', name: 'San Isidro', zone: 'gba1' },
  { id: 'vicente_lopez', name: 'V. L├│pez', zone: 'gba1' },
  { id: 'san_fernando', name: 'San Fernando', zone: 'gba1' },
  { id: 'san_martin', name: 'San Mart├¡n', zone: 'gba1' },
  { id: '3_de_febrero', name: '3 de Febrero', zone: 'gba1' },
  { id: 'hurlingham', name: 'Hurlingham', zone: 'gba1' },
  { id: 'ituzaingo', name: 'Ituzaing├│', zone: 'gba1' },
  { id: 'moron', name: 'Mor├│n', zone: 'gba1' },
  { id: 'avellaneda', name: 'Avellaneda', zone: 'gba1' },
  { id: 'lanus', name: 'Lan├║s', zone: 'gba1' },

  // ÔöÇÔöÇ GBA 2 (middle ring) ÔöÇÔöÇ
  ...
  { id: 'esteban_echeverria', name: 'E. Echeverr├¡a', zone: 'gba2' },
  ...
  // ÔöÇÔöÇ GBA 3 (outer ring) ÔöÇÔöÇ
  { id: 'escobar', name: 'Escobar', zone: 'gba3' },
  { id: 'pilar', name: 'Pilar', zone: 'gba3' },
  { id: 'general_rodriguez', name: 'Gral. Rodr├¡guez', zone: 'gba3' },
  { id: 'marcos_paz', name: 'Marcos Paz', zone: 'gba3' },
  { id: 'canuelas', name: 'Ca├▒uelas', zone: 'gba3' },
  { id: 'san_vicente', name: 'San Vicente', zone: 'gba3' },
  { id: 'pte_peron', name: 'Pte. Per├│n', zone: 'gba3' },
...
export async function renderZones() {
  ...
  section.innerHTML = `
    <div class="section-header">
      <h1 class="section-title">ÔÜÖ´©Å Configuraci├│n de Zonas Flex</h1>
      <p class="section-subtitle">Asign├í partidos a transportistas por zonas</p>
    </div>
  ...
  try {
    ...
    section.innerHTML = `
      <div class="section-header">
        <h1 class="section-title">ÔÜÖ´©Å Configuraci├│n de Zonas Flex</h1>
        <p class="section-subtitle">Asign├í transportistas a cada partido del AMBA</p>
      </div>

      <!-- Carrier Management -->
      <div class="card mb-lg">
        <h3 style="margin-bottom:16px;font-size:16px;font-weight:700;">­ƒÜÜ Transportistas Flex Activos</h3>
        ...
        <div id="carriers-list" style="display:flex;gap:12px;flex-wrap:wrap;">
          ${carriers.length === 0 ? '<p style="color:var(--text-muted);font-size:14px;">Agreg├í al menos un transportista para poder asignar zonas.</p>' : ''}
          ${carriers.map(c => `
            <div style="display:flex;align-items:center;gap:8px;background:var(--surface-hover);padding:8px 14px;border-radius:var(--radius);border:1px solid var(--border);">
              ...
              <button class="btn btn-ghost btn-sm" style="padding:2px 6px;font-size:11px;color:var(--danger);" onclick="window.__deleteCarrier(${c.id})" title="Eliminar">Ô£ò</button>
            </div>
          `).join('')}
        </div>
      </div>
      ...
      <!-- Zones Grid -->
      <h3 style="margin-bottom:16px;font-size:18px;font-weight:700;">­ƒôì Asignaci├│n de Partidos</h3>
      ...
      return `
              ...
              <div style="padding:16px; border-bottom:1px solid var(--border); background: var(--surface-hover);">
                ...
                <div style="display:flex;gap:8px;align-items:center;">
                  <select class="form-select group-carrier-select" id="group-select-${group.id}" style="font-size:12px;flex:1;">
                    <option value="">ÔÇö Asignar a todos ÔÇö</option>
                    ${carrierOptions}
                  </select>
        ...
        return `
                    <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-bottom:1px dashed var(--border);">
                      ...
                      <div style="display:flex;gap:8px;align-items:center;">
                        ${assigned ? `
                          <span class="badge badge-flex" style="font-size:11px;max-width:120px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">­ƒÜÜ ${carrierDisplay}</span>
                          <button class="btn btn-ghost btn-sm btn-remove-zone" data-zone-id="${assigned.zone_id}" style="padding:2px 6px;font-size:11px;color:var(--danger);">Ô£ò</button>
                        ` : `
    ...
    document.getElementById('btn-add-carrier').addEventListener('click', async () => {
      ...
      if (!name) { toast('Ingres├í un nombre', 'error'); return; }
      try {
    ...
    window.__deleteCarrier = async (id) => {
      if (!confirm('┬┐Seguro que quer├®s eliminar este transportista?')) return;
    ...
    document.querySelectorAll('.btn-assign-group').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        ...
        if (!carrier) { toast('Seleccion├í un transportista primero', 'error'); return; }
