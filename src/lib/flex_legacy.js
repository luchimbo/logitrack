...
export async function renderFlex() {
  ...
  section.innerHTML = `
    <div class="section-header">
      <h1 class="section-title">­ƒÜÇ Flex</h1>
      <p class="section-subtitle">Env├¡os por Mercado Env├¡os Flex (despacho propio)</p>
    </div>
  ...
  try {
    ...
    section.innerHTML = `
      <div class="section-header flex-between">
        ...
        <div style="display:flex;gap:8px;">
          <button class="btn btn-primary btn-sm" id="btn-recalc-flex">­ƒöä Recalcular Zonas</button>
          <button class="btn btn-success btn-sm" id="btn-mark-all-flex">Ô£ô Marcar todo encontrado</button>
        </div>
      </div>
      ...
      ${Object.keys(carrierGroups).length > 0 ? `
        <div class="card mb-md">
          <h3 style="margin-bottom:12px;font-size:14px;font-weight:600;color:var(--text-secondary);text-transform:uppercase;letter-spacing:0.5px;">Asignaci├│n de Transportistas Flex</h3>
      ...
      return `
                <div class="carrier-column" style="border:1px solid var(--border);border-radius:var(--radius-lg);overflow:hidden;">
                  <div class="carrier-header" style="background: ${color}20; border-bottom: 2px solid ${color};">
                    <span style="color: ${color}">${isUnassigned ? 'ÔÜá´©Å' : '­ƒÜÜ'} ${carrier}</span>
                  ...
                  <div class="carrier-shipments">
                    ${items.map(s => `
                      <div class="carrier-shipment-item">
                        ...
                        <div class="destination">­ƒôì ${s.city || 'N/A'}, ${s.province || 'N/A'} ┬À CP ${s.postal_code || 'N/A'}</div>
                        <div style="margin-top:4px;color:var(--text-muted);font-size:12px;">­ƒæñ ${s.recipient_name || 'N/A'}</div>
                      </div>
                    `).join('')}
                  </div>
                </div>
      ...
      <h3 style="margin-bottom:16px;font-size:14px;font-weight:600;color:var(--text-secondary);text-transform:uppercase;letter-spacing:0.5px;">
        Lista de Picking ÔÇö Flex
      </h3>

      <div id="flex-picking-items">
        ${flexPicking.length === 0 ? `
          <div class="empty-state">
            <div class="empty-state-icon">­ƒÜÇ</div>
            <p class="empty-state-text">No hay env├¡os Flex.<br>Sub├¡ etiquetas primero.</p>
          </div>
        ` : ''}
      </div>
    `;
    ...
    document.getElementById('btn-recalc-flex')?.addEventListener('click', async () => {
      try {
        ...
        toast(`Ô£à Asignaciones actualizadas (${res.updated} cambios de ${res.total_checked} env├¡os Flex)`, 'success');
        renderFlex(); // re-render the view
      } catch (err) {
        toast('Error al recalcular zonas', 'error');
        document.getElementById('btn-recalc-flex').disabled = false;
        document.getElementById('btn-recalc-flex').innerHTML = '­ƒöä Recalcular Zonas';
      }
    });
