...
export async function renderCarriers() {
  ...
  section.innerHTML = `
    <div class="section-header">
      <h1 class="section-title">­ƒÜÜ Transportistas</h1>
      <p class="section-subtitle">Env├¡os agrupados por transportista y m├®todo de env├¡o</p>
    </div>
  ...
  try {
    ...
    if (!shipments.length) {
      section.innerHTML = `
        ...
        <div class="empty-state">
          <div class="empty-state-icon">­ƒÜÜ</div>
          <p class="empty-state-text">No hay env├¡os cargados.</p>
        </div>
      `;
      return;
    }
    ...
    shipments.forEach(s => {
      ...
      if (s.shipping_method === 'flex') {
        ...
        groupLabel = s.assigned_carrier || 'ÔÜá´©Å Flex Sin Asignar';
        groupColor = s.assigned_carrier ? '#6366f1' : '#ef4444';
      } else {
        groupKey = `colecta-${s.carrier_name || 'desconocido'}`;
        groupLabel = `­ƒôª ${s.carrier_name || 'Colecta'}`;
        groupColor = '#f59e0b';
      }
          ...
          ${group.shipments.map(s => `
            <div class="carrier-shipment-item">
              ...
              <div class="destination">
                ­ƒôì ${s.city || 'N/A'}, ${s.province || 'N/A'} ┬À CP ${s.postal_code || 'N/A'}
              </div>
              <div style="display:flex;justify-content:space-between;align-items:center;margin-top:8px;">
                <span style="color:var(--text-muted);font-size:12px;">
                  ­ƒæñ ${s.recipient_name || 'N/A'}
                </span>
                <select class="status-select" data-id="${s.id}" onchange="window.__updateStatus(${s.id}, this.value)">
                  <option value="pendiente" ${s.status === 'pendiente' ? 'selected' : ''}>ÔÅ│ Pendiente</option>
                  <option value="encontrado" ${s.status === 'encontrado' ? 'selected' : ''}>­ƒöì Encontrado</option>
                  <option value="empaquetado" ${s.status === 'empaquetado' ? 'selected' : ''}>­ƒôª Empaquetado</option>
                  <option value="despachado" ${s.status === 'despachado' ? 'selected' : ''}>Ô£à Despachado</option>
                </select>
              </div>
            </div>
          `).join('')}
    ...
    window.__updateStatus = async (id, status) => {
      try {
        ...
        toast(`Env├¡o #${id} ÔåÆ ${status}`, 'success');
      } catch (err) {
