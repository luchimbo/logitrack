...
export async function renderPicking() {
  ...
  section.innerHTML = `
    <div class="section-header">
      <h1 class="section-title">­ƒôï Lista de Picking</h1>
      <p class="section-subtitle">Productos a buscar ÔÇö divididos por m├®todo de env├¡o</p>
    </div>
  ...
  try {
    ...
    if (!pickingList.length) {
      section.innerHTML = `
        ...
        <div class="empty-state">
          ...
          <p class="empty-state-text">No hay productos para buscar.<br>Sub├¡ etiquetas primero.</p>
        </div>
      `;
      return;
    }
    ...
    section.innerHTML = `
      <div class="section-header flex-between">
        <div>
          <h1 class="section-title">­ƒôï Lista de Picking</h1>
          <p class="section-subtitle">${totalProducts} productos ÔÇö ${totalUnits} unidades (${colectaItems.length} colecta, ${flexItems.length} flex)</p>
        </div>
        <div style="display:flex;gap:8px;">
          <button class="btn btn-success btn-sm" id="btn-mark-all-found">Ô£ô Marcar todo encontrado</button>
        </div>
      </div>
      ...
      ${colectaItems.length > 0 ? `
        <div class="card mb-md" style="border-left: 3px solid #f59e0b;">
          <h3 style="margin-bottom:16px;font-size:16px;font-weight:700;display:flex;align-items:center;gap:8px;">
            ­ƒôª Colecta
            <span class="badge badge-colecta">${colectaItems.length} productos ┬À ${colectaUnits} uds</span>
          </h3>
      ...
      ${flexItems.length > 0 ? `
        <div class="card" style="border-left: 3px solid var(--accent);">
          <h3 style="margin-bottom:16px;font-size:16px;font-weight:700;display:flex;align-items:center;gap:8px;">
            ­ƒÜÇ Flex
            <span class="badge badge-flex">${flexItems.length} productos ┬À ${flexUnits} uds</span>
          </h3>
