...
export function renderUpload() {
  ...
  section.innerHTML = `
    <div class="section-header">
      <h1 class="section-title">­ƒôñ Subir Etiquetas</h1>
      <p class="section-subtitle">Arrastr├í o seleccion├í los archivos ZPL/TXT de etiquetas de MercadoLibre</p>
    </div>

    <div class="upload-zone" id="upload-zone">
      <input type="file" class="upload-input" id="file-input" multiple accept=".txt,.zpl,.TXT,.ZPL" />
      <div class="upload-icon">­ƒôä</div>
      <p class="upload-title">Arrastr├í archivos aqu├¡</p>
      <p class="upload-subtitle">o hac├® click para seleccionar ÔÇö Archivos .txt o .zpl</p>
    </div>
...
async function handleFiles(fileList) {
  ...
  try {
    ...
    toast(`Ô£à ${result.total_parsed} nuevos env├¡os agregados al lote del d├¡a (${result.total_in_batch} total)${result.total_skipped ? ` ÔÇö ${result.total_skipped} duplicados omitidos` : ''}`, 'success');
...
function showUploadResults(result) {
  ...
  container.innerHTML = `
    <div class="card" style="border-color: var(--success); border-left: 4px solid var(--success);">
      <h3 style="margin-bottom:16px;">Ô£à Lote del d├¡a ÔÇö ${new Date().toLocaleDateString('es-AR')}</h3>
      <div class="stats-grid">
        <div class="stat-card card accent">
          <div class="stat-value">${result.total_in_batch}</div>
          <div class="stat-label">Total en el d├¡a</div>
        </div>
      ...
      ${result.total_skipped > 0 ? `
        <p style="color:var(--text-muted);font-size:13px;margin-bottom:12px;">
          ÔÜá´©Å ${result.total_skipped} env├¡o${result.total_skipped > 1 ? 's' : ''} duplicado${result.total_skipped > 1 ? 's' : ''} omitido${result.total_skipped > 1 ? 's' : ''} (ya estaban cargados)
        </p>
      ` : ''}
