...
Handles two distinct label formats:
  ...
  - FLEX: Labels for Mercado Env├¡os Flex (own delivery / carrier pickup)
"""
...
LOCALITY_TO_PARTIDO = {
    ...
    # Monterrey is likely not a GBA location ÔÇö add as unknown
}


def decode_zpl_hex(text: str) -> str:
    """Decode ZPL hex-encoded UTF-8 characters like _C3_B3 -> ├│, _2E -> ."""
...
def is_flex_label(content: str) -> bool:
    ...
    # Flex labels explicitly say "Env├¡o Flex" or "Envio Flex"
...
def parse_colecta_label(content: str) -> Optional[dict]:
    ...
    # --- Carrier detection (CAJAS V4 ÔåÆ specific carrier like OCASA/PICKIT) ---
