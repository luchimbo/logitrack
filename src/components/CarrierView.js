...
export default function CarrierView() {
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            setError(null);
            try {
                const data = await api(`/shipments?batch_id=${currentBatchId}`);
                setShipments(data);
            } catch (err) {
        ...
        return (
            <div className="section active">
                <div className="section-header">
                    ...
                    <p className="section-subtitle">Envíos agrupados por transportista y método de envío</p>
                </div>
            ...
            <div className="stats-grid">
                ...
                {unassignedFlex > 0 && (
                    <div className="stat-card card danger">
                        <div className="stat-value">{unassignedFlex}</div>
                        <div className="stat-label">Flex Sin Asignar</div>
                    </div>
                )}
            </div>
                ...
                {sortedGroups.map(([key, group]) => (
                    <div key={key} className="carrier-column card">
                        <div className="carrier-header" style={{ background: `${group.color}20`, borderBottom: `2px solid ${group.color}` }}>
                            ...
                            <span className="carrier-count" style={{ background: `${group.color}30`, color: group.color }}>
