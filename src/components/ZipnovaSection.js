  ...
  return (
    <section className="section">
      <div className="section-header">
        ...
        <p className="section-subtitle">Apartado separado para consultar envíos de Zipnova. Visible solo para el admin maestro.</p>
      </div>
      ...
      <div className="card" style={{ marginBottom: '18px' }}>
        <div className="form-row">
          <div className="form-group">
            ...
            <input className="form-input" value={status} onChange={(e) => setStatus(e.target.value)} placeholder="new, delivered, etc" />
          </div>
          <div className="form-group">
            <label className="form-label">Service type</label>
            <input className="form-input" value={serviceType} onChange={(e) => setServiceType(e.target.value)} placeholder="standard_delivery" />
          </div>
          <div className="form-group">
            <label className="form-label">External ID</label>
            <input className="form-input" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar por external_id" />
          </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(320px, 420px) minmax(0, 1fr)', gap: '20px', alignItems: 'start' }}>
        <div className="card">
          <div className="flex-between mb-md">
            <h3 style={{ fontSize: '16px', fontWeight: 700 }}>Envíos Zipnova</h3>
            <span style={{ color: 'var(--text-muted)', fontSize: '12px' }}>Total página: {shipments.length}</span>
          </div>
            ...
            {shipments.map((shipment) => (
              ...
              >
                ...
                <div className="mobile-card-body" style={{ marginTop: '8px' }}>
                  ...
                  <div className="mobile-card-row"><span className="mobile-card-label">Tipo</span><span className="mobile-card-value">{shipment.logistic_type} / {shipment.service_type}</span></div>
                  <div className="mobile-card-row"><span className="mobile-card-label">Paquetes</span><span className="mobile-card-value">{shipment.total_packages}</span></div>
                </div>
              </button>
            ))}
            {!loading && shipments.length === 0 && <p style={{ color: 'var(--text-muted)', fontSize: '13px' }}>No hay envíos para los filtros elegidos.</p>}
          </div>
        ...
        <div className="card">
          <div className="flex-between mb-md">
            <h3 style={{ fontSize: '16px', fontWeight: 700 }}>Detalle del envío</h3>
          ...
          {detailLoading ? (
            ...
            <div style={{ display: 'grid', gap: '14px' }}>
              <div className="stats-grid">
                ...
                <div className="stat-card card info"><div className="stat-value">{selectedDetail.total_weight || 0}</div><div className="stat-label">Peso total</div></div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '16px' }}>
                <div className="card">
                  <h4 style={{ fontSize: '15px', fontWeight: 700, marginBottom: '12px' }}>Resumen</h4>
                  <div style={{ display: 'grid', gap: '8px' }}>
                    <div className="mobile-card" style={{ display: 'block', marginBottom: 0 }}>
                      <div className="mobile-card-row"><span className="mobile-card-label">External ID</span><span className="mobile-card-value">{selectedDetail.external_id || '-'}</span></div>
                      <div className="mobile-card-row"><span className="mobile-card-label">Delivery ID</span><span className="mobile-card-value">{selectedDetail.delivery_id || '-'}</span></div>
                ...
                <div className="card">
                  <h4 style={{ fontSize: '15px', fontWeight: 700, marginBottom: '12px' }}>Servicio</h4>
                  <div style={{ display: 'grid', gap: '8px' }}>
                    <div className="mobile-card" style={{ display: 'block', marginBottom: 0 }}>
                      ...
                      <div className="mobile-card-row"><span className="mobile-card-label">Tracking</span><span className="mobile-card-value">{selectedDetail.tracking || '-'}</span></div>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="empty-state" style={{ padding: '30px 12px' }}>
              ...
              <p className="empty-state-text">Elegí un envío de Zipnova para ver su detalle completo.</p>
            </div>
          )}
        </div>
      </div>
